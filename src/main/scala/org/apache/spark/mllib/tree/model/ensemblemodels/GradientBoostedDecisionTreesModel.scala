 ...
 */
@Experimental
class GradientBoostedDecisionTreesModel(
    override val algo: Algo,
    override val trees: Array[OptimizedDecisionTreeModel],
  ...
  override def save(sc: SparkContext, path: String): Unit = {
    TreeEnsembleModel.SaveLoadV1_0.save(sc, path, this,
      GradientBoostedDecisionTreesModel.SaveLoadV1_0.thisClassName)
  }
  ...
  def evaluateEachIteration(
    ...
    var predictionAndError = GradientBoostedDecisionTreesModel.computeInitialPredictionAndError(
  ...
  override protected def formatVersion: String = GradientBoostedDecisionTreesModel.formatVersion
}

object GradientBoostedDecisionTreesModel extends Loader[GradientBoostedDecisionTreesModel] {
  ...
  def computeInitialPredictionAndError(
      ...
      initTree: OptimizedDecisionTreeModel,
  ...
  def updatePredictionError(
    ...
    treeWeight: Double,
    tree: OptimizedDecisionTreeModel,
    loss: Loss): RDD[(Double, Double)] = {
  ...
  override def load(sc: SparkContext, path: String): GradientBoostedDecisionTreesModel = {
    ...
    (loadedClassName, version) match {
      case (className, "1.0") if className == classNameV1_0 =>
        ...
        new GradientBoostedDecisionTreesModel(Algo.fromString(metadata.algo), trees, metadata.treeWeights)
      case _ => throw new Exception(s"GradientBoostedDecisionTreesModel.load did not recognize model" +
  ...
  private object SaveLoadV1_0 {
    ...
    def thisClassName: String = "org.apache.spark.mllib.tree.model.GradientBoostedDecisionTreesModel"
  }
...
private[tree] sealed class TreeEnsembleModel(
    ...
    protected val trees: Array[OptimizedDecisionTreeModel],
  ...
  def toDebugString: String = {
    ...
    header + trees.zipWithIndex.map { case (tree, treeIndex) =>
          val numOfInternalNodes = tree.topNode.internalNodes
      s"  [Evaluator:$treeIndex]\n EvaluatorType=DecisionTree \n" +
...
private[tree] object TreeEnsembleModel extends Logging {
  ...
  object SaveLoadV1_0 {
    ...
    import org.apache.spark.mllib.tree.model.opdtmodel.OptimizedDecisionTreeModel.SaveLoadV1_0.{TreeNodeData, constructTrees}
    ...
    def save(sc: SparkContext, path: String, model: TreeEnsembleModel, className: String): Unit = {
      ...
      val dataRDD = sc.parallelize(model.trees.zipWithIndex).flatMap { case (tree, treeId) =>
        tree.topNode.subtreeIterator.toSeq.map(node => TreeNodeData(treeId, node))
      }.toDF()
      logInfo("flag test")
