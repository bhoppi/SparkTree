...
package org.apache.spark.mllib.tree.model.node
...
class Node (
  ...
  def setId(NewInt: Int):Unit = {id =  NewInt}
...
/*  Get the number of internal nodes */
  private[tree] def internalNodes: Int = if (isLeaf) {
    0
  } else {
    1 + leftNode.get.internalNodes + rightNode.get.internalNodes
  }
